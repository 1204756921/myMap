...
export default class WmtsImageLayer extends ImageLayer {
    constructor(options) {
        super();
        this.id = defaultValue(options.id, (new Date()).getTime());
        this.name = defaultValue(options.name, 'wmtsMap');
        this._show = defaultValue(options.show, true);
        this.url = options.url;
        this.layers = options.layers;
        this.style = defaultValue(options.style, 'default');
        this.tileMatrixSetID = options.tileMatrixSetID
        this.type = "WmtsImageLayer";
    }
    set show(val) {
        this._show = val;
        this.layer.show = val;
    ...
    addTo(map) {
        this.map = map;
        this.layer = this.map.Viewer.imageryLayers.addImageryProvider(new WebMapTileServiceImageryProvider({
            url: this.url,
            layers: this.layers,
            style: this.style,
            format : 'image/png',
